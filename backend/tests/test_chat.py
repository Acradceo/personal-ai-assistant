...
def test_chat_happy_path(client, mocker):
    mocker.patch('backend.app.API_KEY', 'test-key')
    ...
    response = client.post('/api/chat', json={"message": "Hello"}, headers={'X-API-Key': 'test-key'})
...
def test_chat_empty_message(client, mocker):
    mocker.patch('backend.app.API_KEY', 'test-key')
    response = client.post('/api/chat', json={"message": ""}, headers={'X-API-Key': 'test-key'})
...
def test_chat_unauthorized(client, mocker):
    mocker.patch('backend.app.API_KEY', 'test-key')
    ...
    # Incorrect API Key
    response = client.post('/api/chat', json={"message": "Hello"}, headers={'X-API-Key': 'wrong-key'})
    assert response.status_code == 401
